...
void spso(size_t nDim, /*!< Number of search dimensions */
	...
	double cur_vi; //stores velocity for velocity clamping
...
#pragma omp parallel for
        ...
        for (lpParticles = 0; lpParticles < popsize; lpParticles++) {
			...
			//apply confinement. Velocity clamping
            for (lpDimentions = 0; lpDimentions<nDim; lpDimentions++) {
                cur_vi = gsl_vector_get(pop[lpParticles].partVel, lpDimentions);
                if (abs(cur_vi) > psoParams->max_velocity) { //if particle is above upper bound
                    gsl_vector_set(pop[lpParticles].partVel, lpDimentions, GSL_SIGN(cur_vi)*psoParams->max_velocity);
                } // end if
